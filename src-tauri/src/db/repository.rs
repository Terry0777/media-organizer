...
use chrono::Utc;
use log::info;
use rusqlite::{params, types::ToSql, Connection, OptionalExtension, Result};
...
impl MediaRepository {
    ...
    pub fn list(
        ...
        // 修改点：使用 Vec<Box<dyn ToSql>> 来拥有数据的所有权
...
impl AlbumMediaRepository {
    ...
    /// Get media for album
    pub fn get_media_for_album(conn: &Connection, album_id: i64) -> Result<Vec<MediaFile>> {
        let mut stmt = conn.prepare(
            "SELECT mf.id, mf.file_path, mf.file_type, mf.file_size, mf.width, mf.height,
                    mf.duration, mf.created_at, mf.modified_at, mf.taken_at, mf.device,
                    mf.gps_lat, mf.gps_lon, mf.checksum, mf.thumbnail_path, mf.is_deleted
